...
const rIp = (octet1, octet2, octet3) =>
  `${octet1}.${octet2}.${octet3}.${Math.floor(Math.random() * 254) + 1}`;
const pick = (arr) => arr[Math.floor(Math.random() * arr.length)];
const IFS = {
  router: ['Gi0/0/0', 'Gi0/0/1', 'Gi0/0/2', 'Te0/0/0/0', 'Lo0', 'Mgmt0'],
  switch: ['Eth1/1', 'Eth1/2', 'Eth1/3', 'Eth1/4', 'Eth1/5', 'Eth1/6', 'Vlan1'],
...
const roleForDeviceIndex = (d) => {
  if (d < 5) return 'core';
  if (d < 20) return 'distribution';
  return 'access';
};
...
async function main() {
      ...
      for (let i = 0; i < ifs.length; i++) {
        ...
        docs.push({
          ...
          interface: {
            ...
            errors: {
              in: down ? Math.floor(Math.random() * 50) : 0,
  ...
  for (let i = 0; i < hqRouters.length; i++) {
    for (let j = i + 1; j < Math.min(i + 3, hqRouters.length); j++) {
      const rA = hqRouters[i];
      const rB = hqRouters[j];
      docs.push({
        '@timestamp': ts,
        ...
        bgp_peer: {
          ...
          prefixes_received: Math.floor(Math.random() * 50000) + 10000,
          prefixes_sent: 500,
          uptime_seconds: Math.floor(Math.random() * 2592000) + 3600,
          in_updates: Math.floor(Math.random() * 10000),
          out_updates: Math.floor(Math.random() * 1000),
        },
      });
    }
  }
    ...
    if (rtr) {
      ...
      docs.push({
        ...
        bgp_peer: {
          ...
          prefixes_sent: 1000,
          uptime_seconds: Math.floor(Math.random() * 2592000),
          in_updates: Math.floor(Math.random() * 100000),
          out_updates: Math.floor(Math.random() * 5000),
        },
      });
    }
  }
      ...
      for (let j = i + 1; j < Math.min(i + 4, routers.length); j++) {
        ...
        docs.push({
          ...
          ospf_neighbor: {
            ...
            priority: 1,
            dead_timer: 40,
            retrans_count: Math.floor(Math.random() * 10),
          },
        });
      }
    }

...
const TRANSIT_PROVIDERS = [
  { asn: 3356,  name: 'Lumen',        ip: '198.51.100.1' },
  { asn: 174,   name: 'Cogent',       ip: '198.51.100.5' },
  { asn: 6939,  name: 'Hurricane Electric', ip: '198.51.100.9' },
];
const BRANCH_ISPS = [
  { asn: 7922,  name: 'Comcast',      ip: '203.0.113.1' },
  { asn: 20115, name: 'Charter',      ip: '203.0.113.5' },
];
...
function makeOspfDoc(ts, localDev, remoteIp, routerId, state, areaId, priority) {
  ...
  return {
    ...
    ospf_neighbor: {
      ...
      priority,
      dead_timer: 40,
      retrans_count: state === 'Full' ? Math.floor(Math.random() * 5) : Math.floor(Math.random() * 50) + 10,
    },
  };
}

async function bulk(docs){const body=docs.flatMap(d=>[{create:{_index:IDX}},d]);const r=await fetch(`${ES}/_bulk`,{method:'POST',headers:{'Content-Type':'application/x-ndjson',Authorization:`Basic ${AUTH}`},body:body.map(l=>JSON.stringify(l)).join('\n')+'\n'});const j=await r.json();if(j.errors){const e=j.items.find(i=>i.create?.error);if(e)console.error('Err:',JSON.stringify(e.create.error))}return j;}
...
const all=[...docs,...arp,...mac,...bgp,...ospf];console.log(`Total docs: ${all.length} (${bgp.length} BGP, ${ospf.length} OSPF)`);

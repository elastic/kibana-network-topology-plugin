...
export const DeviceFlyout: React.FC<Props> = ({ deviceId, onClose, from, to }) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .fetchDeviceDetail(deviceId, { from, to })
      .then((r) => {
        if (!c) {
          setData(r);
          setLoading(false);
    ...
    <EuiFlyout onClose={onClose} size="m" ownFocus>
          ...
          <>
            <EuiFlexGroup alignItems="center" gutterSize="m">
              <EuiFlexItem grow={false}>
                <EuiBadge color={DEVICE_TYPE_CONFIG[data.device.type]?.color || '#98A2B3'}>
                  {data.device.type}
                </EuiBadge>
              </EuiFlexItem>
              <EuiFlexItem>
              ...
              <EuiFlexItem grow={false}>
                <EuiHealth color={STATUS_EUI_COLORS[data.device.status] || 'subdued'}>
                  {data.device.status}
                </EuiHealth>
              </EuiFlexItem>
            </EuiFlexGroup>
            ...
            <EuiDescriptionList
              ...
              listItems={[
                ...
                {
                  title: 'Interfaces',
                  description: `${data.device.interfaceCount} (${data.device.downInterfaceCount} down)`,
                },
              ]}
            />
          </>
      ...
      <EuiFlyoutBody>
        ...
        {data && (
          <>
            <EuiTabs size="s">
              <EuiTab isSelected={tab === 'interfaces'} onClick={() => setTab('interfaces')}>
                Interfaces ({data.interfaces.length})
              </EuiTab>
              <EuiTab isSelected={tab === 'neighbors'} onClick={() => setTab('neighbors')}>
                ARP Neighbors ({data.neighbors.length})
              </EuiTab>
              {data.bgpPeers.length > 0 && (
                <EuiTab isSelected={tab === 'bgp'} onClick={() => setTab('bgp')}>
                  BGP Peers ({data.bgpPeers.length})
                </EuiTab>
              )}
              {data.ospfNeighbors.length > 0 && (
                <EuiTab isSelected={tab === 'ospf'} onClick={() => setTab('ospf')}>
                  OSPF ({data.ospfNeighbors.length})
                </EuiTab>
              )}
            </EuiTabs>
            <EuiSpacer size="m" />
            ...
            {tab === 'neighbors' && (
              <EuiBasicTable
                items={data.neighbors}
                compressed
                columns={[
                  { field: 'ip', name: 'IP Address' },
                  { field: 'mac', name: 'MAC Address' },
                ]}
              />
            )}
            {tab === 'bgp' && (
                ...
                columns={[
                  {
                    ...
                    width: '100px',
                    render: (s: string) => (
                      <EuiHealth color={BGP_EUI_COLORS[s] || 'subdued'}>{s}</EuiHealth>
                    ),
                  },
                ...
                columns={[
                  {
                    ...
                    width: '90px',
                    render: (s: string) => (
                      <EuiHealth color={OSPF_EUI_COLORS[s] || 'subdued'}>{s}</EuiHealth>
                    ),
                  },
